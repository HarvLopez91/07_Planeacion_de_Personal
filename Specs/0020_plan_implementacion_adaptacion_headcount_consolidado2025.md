...
Durante la validación se detectaron errores de refresco (`DataFormat.Error: Valor de celda '#N/A' no válido`) en `Consolidado2025`/`PLANTA DE PERSONAL`. Perfilado de solo lectura contra el Excel (`Data/HeadCount/2025/Consolidado 2025.xlsx`) confirmó, con conteo exacto que coincide con el total reportado por Power BI Desktop en un intento de refresco (295 filas afectadas; en un intento posterior el conteo bajó a 36, variabilidad atribuible al estado de cálculo de fórmulas del Excel vivo en SharePoint, no a un defecto determinístico):
