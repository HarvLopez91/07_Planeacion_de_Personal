...
| DATA-012 | Adaptar HeadCount/`Consolidado 2025.xlsx` a su nueva estructura (68 columnas) | El Excel fue actualizado manualmente con columnas nuevas (Generación real, Rango de Edad, Árbol de Nómina Nivel 2/3, Tipo Identificación, COD. CARGO, Tipo Contrato (Kactus), Indicador Actividad, Año Nac); el Power Query de `Consolidado2025`/`PLANTA DE PERSONAL` no está adaptado y produce el error `NIVEL_DE_CARGO` reportado por el usuario. Existe además un parche local sin commitear en el working tree principal, parcialmente aplicable (ver `Specs/0019` sección 2), con una regresión de codificación de Unión Libre que no debe reaplicarse | Datos / Modelo | Alta | Ninguna | Formula Firewall en `PLANTA DE PERSONAL` (riesgo ya documentado); posible desincronización entre el Excel local y la copia publicada en SharePoint corporativo; sin homologación inequívoca entre Árbol de Nómina Nivel 2/3 y Nivel/Tipo de Cargo | Resolver las 6 decisiones humanas pendientes de `Specs/0019` antes de elaborar un plan de implementación |
