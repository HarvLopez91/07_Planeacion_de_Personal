...
| DATA-012 | Adaptar HeadCount/`Consolidado 2025.xlsx` a su nueva estructura (68 columnas) | El Excel fue actualizado manualmente con columnas nuevas (Generación real, Rango de Edad, Árbol de Nómina Nivel 2/3, Tipo Identificación, COD. CARGO, Tipo Contrato (Kactus), Indicador Actividad, Año Nac); el Power Query de `Consolidado2025`/`PLANTA DE PERSONAL` no está adaptado, la falsa equivalencia histórica `RANGO DE EDAD → GENERACIÓN` ya no es válida para 2025 (evidencia visual confirmada, ver `Specs/0019` sección 4-bis) y hay una hipótesis sin confirmar sobre el error `NIVEL_DE_CARGO` (posible desincronización entre el Excel local y la fuente remota real, OneDrive personal — no SharePoint corporativo, ver `Specs/0019` sección 4). Existe además un parche local sin commitear en el working tree principal, parcialmente aplicable (ver `Specs/0019` sección 2), con una regresión de codificación de Unión Libre que no debe reaplicarse | Datos / Modelo | Alta | PR #7 (Unión Libre) debe estar fusionado a `main` antes de implementar | Formula Firewall en `PLANTA DE PERSONAL` (riesgo ya documentado); posible desincronización entre el Excel local y la copia de OneDrive personal realmente consumida por `main` (no hay copia en SharePoint corporativo vigente); sin homologación inequívoca entre Árbol de Nómina Nivel 2/3 y Nivel/Tipo de Cargo | Obtener aprobación de las decisiones recomendadas A-E y verificar la sincronización de la fuente remota (`Specs/0019` sección 10) antes de elaborar un plan de implementación |
