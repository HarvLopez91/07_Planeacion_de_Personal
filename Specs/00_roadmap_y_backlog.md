...
| Fecha de última actualización de este roadmap | 2026-08-10 (tarde) |
...
|---|---|---|---|---|---|---|---|
| DATA-005 | [ABSORBIDA POR DATA-012] Migración de `AREAS` a fuente corporativa SharePoint | **Absorbida por `DATA-012`** (decisión F, `Specs/0019` sección 9.8): la migración de `AREAS` a SharePoint corporativo ya fue aprobada como parte del alcance de `DATA-012`, junto con `Consolidado2025` y `PLANTA DE PERSONAL` — las 3 comparten el mismo cambio de patrón de fuente. No ejecutar `DATA-005` como iniciativa independiente; no está Finalizada porque el cambio aún no está versionado en `main` (sigue solo en un worktree temporal de verificación, sin commitear) | Datos | Media | `DATA-012` | Ninguno propio — los riesgos de esta migración quedan documentados bajo `DATA-012` (`Specs/0019` sección 11) | No aplica ejecución independiente. Su implementación efectiva queda cerrada cuando `DATA-012` integre la migración de `AREAS` a `main` — no requiere Spec ni plan propio |
...
| DATA-012 | Adaptar HeadCount/`Consolidado 2025.xlsx` a su nueva estructura (68 columnas) y migrar `Consolidado2025`/`PLANTA DE PERSONAL`/`AREAS` a SharePoint corporativo | El Excel fue actualizado manualmente con columnas nuevas (Generación real, Rango de Edad, Árbol de Nómina Nivel 2/3, Tipo Identificación, COD. CARGO, Tipo Contrato (Kactus), Indicador Actividad, Año Nac); el Power Query de `Consolidado2025`/`PLANTA DE PERSONAL` no está adaptado y la falsa equivalencia histórica `RANGO DE EDAD → GENERACIÓN` ya no es válida para 2025 (evidencia visual confirmada, `Specs/0019` sección 4-bis). Causa del error `NIVEL_DE_CARGO` ya CONFIRMADA y verificada empíricamente: la fuente antigua (OneDrive personal) estaba desincronizada; la migración a la biblioteca corporativa (`lemcosas.sharepoint.com/sites/TalentoHumanoGrupoLemco/...`) resuelve el error (`Specs/0019` sección 4/9.8). Decisiones A-E y F ya **aprobadas** por el usuario (`Specs/0019` sección 10). Existe además un parche local sin commitear en el working tree principal, parcialmente aplicable (ver `Specs/0019` sección 2), con una regresión de codificación de Unión Libre que no debe reaplicarse; y un worktree temporal de verificación (`.wt/data012-verificacion-fuente`) con la migración de fuente aplicada manualmente pero mezclada con ruido de Desktop — ninguno de los dos debe reutilizarse tal cual | Datos / Modelo | Alta | PR #7 (Unión Libre) debe estar fusionado a `main` antes de implementar | Formula Firewall/niveles de privacidad en `PLANTA DE PERSONAL`, `Selección Grupo Lemco` y `SENA UNIDADES` (riesgo reportado por el usuario, aún no resuelto, `Specs/0019` sección 11); sin homologación inequívoca entre Árbol de Nómina Nivel 2/3 y Nivel/Tipo de Cargo (diferida a iniciativa futura, decisión C) | Elaborar el plan de implementación aplicando la migración de fuente (F) y las decisiones A-E sobre una base `main` + PR #7 fusionado (`Specs/0019` sección 13); no quedan decisiones humanas pendientes para el alcance mínimo |
